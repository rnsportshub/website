...
function _getFiltered() {
  ...
  // Jersey Type (fan / player)
  if (_state.jerseyType !== 'all') {
    r = r.filter(p => (p.type || '').toLowerCase() === _state.jerseyType);
  }
...
function _renderActiveTags() {
  ...
  if (_state.jerseyType !== 'all') {
    const label = _state.jerseyType === 'fan' ? '👕 Fan Version' : '⭐ Player Version';
    tags.push({ label, clear: () => { _state.jerseyType = 'all'; _state.page = 1; _render(); }});
  }
...
function _readSidebar() {
  ...
  const jt = document.querySelector('input[name="jersey-type"]:checked');
  if (jt) _state.jerseyType = jt.value;
...
function _writeSidebar() {
  ...
  const jtEl = document.querySelector(`input[name="jersey-type"][value="${_state.jerseyType}"]`);
...
function _writeChips() {
  document.querySelectorAll('.filter-chip').forEach(c => {
    ...
    let active = false;
    if (f === 'all') {
      active = _state.cat === 'all' && !_state.brands.length && _state.jerseyType === 'all' && !_state.search;
    } else if (['jerseys','studs','gear'].includes(f)) {
      active = _state.cat === f && !_state.brands.length && _state.jerseyType === 'all';
    } else if (f === 'fan' || f === 'player') {
      active = _state.jerseyType === f;
    } else {
      // brand chip
...
window.applyFilter = function(val, btn) {
  ...
  if (val === 'all') {
    _state.cat = 'all'; _state.brands = []; _state.jerseyType = 'all'; _state.search = '';
  } else if (val === 'fan' || val === 'player') {
    _state.jerseyType = val;
...
window.clearFilters = function() {
  _state.cat = 'all'; _state.brands = []; _state.jerseyType = 'all'; _state.priceMin = 0;
...
(function() {
  ...
  if (type && (type === 'fan' || type === 'player')) _state.jerseyType = type;
})();
